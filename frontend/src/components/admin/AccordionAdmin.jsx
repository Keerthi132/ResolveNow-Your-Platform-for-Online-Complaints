...
const AccordionAdmin = () => {
  ...
  useEffect(() => {
    ...
    const getAgentsRecords = async () => {
      try {
        const response = await axios.get('http://localhost:5000/AgentUsers');
  ...
  const handleSelection = async (agentId, complaintId, status, agentName) => {
    try {
      await axios.get(`http://localhost:5000/AgentUsers/${agentId}`);
      ...
      await axios.post('http://localhost:5000/assignedComplaints', assignedComplaint);
